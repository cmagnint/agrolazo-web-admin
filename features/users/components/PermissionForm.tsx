...
import type { PermissionActionState, PermissionFormValues } from "../schemas";

type PermissionFormMode = "create" | "edit";

type PermissionFormProps = {
  action: (
    previousState: PermissionActionState,
    formData: FormData,
  ) => Promise<PermissionActionState>;
  initialState: PermissionActionState;
  mode: PermissionFormMode;
};

export function PermissionForm({ action, initialState, mode }: PermissionFormProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1">
        <h2 id="permission-form-title" className="text-lg font-bold tracking-normal">
          {isEdit ? "Editar permiso" : "Nuevo permiso"}
        </h2>
        <p className="text-sm text-[var(--color-text-muted)]">
          {isEdit ? "Actualiza los datos del permiso." : "Crea un permiso para el administrador autenticado."}
        </p>
      </div>
      ...
      <form action={formAction} className="mt-6 space-y-5" noValidate>
        ...
        <div className="flex justify-end">
          <Button type="submit" disabled={pending}>
            {pending ? "Guardando..." : isEdit ? "Guardar cambios" : "Crear permiso"}
          </Button>
        </div>
      </form>
    </section>
...
function TextField({
  ...
  error?: string;
  label: string;
  name: keyof Pick<PermissionFormValues, "description" | "name">;
  value: string;
